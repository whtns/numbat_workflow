...
qc_dir <- "~/single_cell_pipeline/output/FACS_20171031_dshayler_H_sapiens_output/multiqc_data/"

qc_file_names <- c("multiqc_rseqc_read_distribution", "multiqc_picard_dups", "multiqc_cutadapt", "multiqc_bowtie2", "mqc_mqc_mplplot_eygcjksmfb_1")
...
write.table(scone_qc, scone_qc_path, sep = "\t")
...
assay(dshayler_2seq) = assays(dshayler_2seq)$counts 


rnaseq_metrics <- read.table(scone_qc_path, sep = "\t", header = TRUE)
...
metadata(dshayler_2seq)$which_qc <- colnames(rnaseq_metrics)
...
table(colData(dshayler_2seq)$Seq_Number,
      colData(dshayler_2seq)$Fetal_Age)
