...
convert_mt_cells <- function(cell_settings){
  # browser()
  ...
  treatment_groups <- c("sh733", "sh737", "sh842", "shCtrl")
  treat_idx <- which(grepl(paste(treatment_groups, collapse="|"), colnames))
  ...
  empty <- which(sapply(sub_vecs, function(x) !length(x)))
  sub_vecs <- sub_vecs[-empty]
...
annotation <- convert_mt_cells(opt$cellset)
