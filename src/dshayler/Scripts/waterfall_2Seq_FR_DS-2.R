...
#alt gene symbol ID from column 1 not rowname
genesfromtranscript <- function(tx_df){
...
traj1 <- "~/single_cell_pipeline/results/dominic/waterfall trajectories/051618_Traj1_cell_psuedo_correl.csv"
traj2 <- "~/single_cell_pipeline/results/dominic/waterfall trajectories/traj2_cell_psuedo_correl.csv"
traj3 <- "~/single_cell_pipeline/results/dominic/waterfall trajectories/traj3_cell_psuedo_correl.csv"
fix_traj <- function(traj_file, new_traj_file){
  traj0<-read.csv(traj_file)
  traj0 <- traj0[,-1]
  traj0 <- traj0[complete.cases(traj0),]
  gene_ids <- genesfromtranscript(traj0)
  traj0_f <- cbind(traj0, names(gene_ids))
  write.csv(traj0_f, new_traj_file)
  return(traj0_f)
}

new_traj <- fix_traj(traj3, gsub('.csv', '_new.csv', traj3))
...
out_PDF="~/single_cell_pipeline/results/dominic/TwoSeq_noVSX_AKT_061918.pdf"
pdf(out_PDF, title="TwoSeq_noVSX_AKT_061918")
...
seq_grp1<-rownames(cell_groups[cell_groups["Seq_Number"]=="1",])
week18<-rownames(cell_groups[cell_groups["Fetal_Age"]=="18",])
...
trajectory3_cols=c("lightblue","lightgreen")
...
traj3_col_vect <- cell_groups_reduced$Cluster_Color[cell_groups_reduced$Cluster_Color %in% trajectory3_cols]
traj3_col_vect <- gsub("lightgreen", "green", traj3_col_vect)
...
T3_list<-cell_groups_reduced %>%
  dplyr::filter(Cluster_Color %in% c("lightblue", "lightgreen")) %>% 
  pull(Sample_ID) %>% 
  as.character()
...
#for trajectory
jt.PT = Waterfall.Pseudotime(jt.l_tmp, angle=0, col = traj2_col_vect, plot_title = "TwoSeq_Traj2_052418.pdf", 
                             nclust = 5, seed = 5,scalePCA = F,mst=FALSE,invert_pt = FALSE, 
                             label=TRUE, threeD_plot = TRUE, Keep=T2_list)

jt.PT = Waterfall.Pseudotime(jt.l_tmp, angle=0, col = traj1_col_vect, plot_title = "TwoSeq_Traj1_052418.pdf", 
                             nclust = 5, seed = 5,scalePCA = F,mst=FALSE,invert_pt = FALSE, 
                             label=TRUE, threeD_plot = TRUE, Keep=T1_list)
jt.PT = Waterfall.Pseudotime(jt.l_tmp, angle=0, col = traj3_col_vect, plot_title = "TwoSeq_Traj1_052418.pdf", 
                             nclust = 5, seed = 5,scalePCA = F,mst=FALSE,invert_pt = FALSE, 
                             label=TRUE, threeD_plot = TRUE, Keep=T3_list)
...
out_PDF="/dataVolume/storage/single_cell_pipeline/results/dominic/052418_putative_traj_plots.pdf"
pdf(out_PDF, title="052418_putative_traj_plots")
...
color_exp_plot<- plot_cell_colors(akt, jt)
...
plot_transcript_colors <- function(marker_set, dataset){
  ...
  for (i in marker_set){
    ...
    plot(coords,col = trans_vect,main = i, pch=19)
    text(coords[,1],coords[,2],names(TargetGene), cex=0.3) 
    transcript_colors <- cbind(transcript_colors, i = trans_vect)
  }
...
write.table(as.data.frame(pt_cor_up),"./traj3_cell_psuedo_correl.csv", sep = ",")
write.table(as.data.frame(pt_cor_dn), "./traj3_cell_psuedo_correl.csv", sep = ",", append = TRUE)
test <-  read.table("./traj3_cell_psuedo_correl.csv", sep = ",")
