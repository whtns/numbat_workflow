...
EXPRESSION_PATH = "/dataVolume/storage/single_cell_pipeline/output/FACS_20171031_dshayler_H_sapiens_output/transcripts.tpm_census_matrix.csv" #this points to your gene expression file 
GROUP_PATH = "/dataVolume/storage/single_cell_pipeline/data/sc_cone_devel/sc_cone_devel_H_sapiens/FACS_20171031_dshayler_H_sapiens/Sample_number_attributes_103117.csv"
...
out_PDF="/dataVolume/storage/single_cell_pipeline/results/dominic/110917_poorcells.pdf"
pdf(out_PDF, title="110917_remove_poorcells")

cell_groups = read.csv(GROUP_PATH, header = TRUE, row.names=1,  colClasses=c("character","character","character","character","character","character"))
...
age_cols = c("blue","green","red","black")
names(age_cols) = c("13","15","18","19")
group_cols = c("purple", "green", "blue","yellow","red")
names(group_cols)=c("1","2","3","4","5")
...
cells_of_interest= which(names(col_vect) %in% poor_reads)
col_vect[cells_of_interest]="red"
#run Waterfall.PCA() from new waterfall script 05/17/2017

#get PCA data object w/ cool plot!!
jt.PCA = Waterfall.PCA(TPM_Data = jt.l_tmp, col = col_vect, plot_title = "poor reads", twoDcomp = c(1,2)) 
...
jt.PT = Waterfall.Pseudotime(jt.l_tmp, angle=0, col = jt.cols, plot_title = "103117_FR_removed_bad.pdf", 
                             nclust = 5, seed = 5,scalePCA = F,mst=FALSE,invert_pt = FALSE, 
                             label=TRUE,threeD_plot = FALSE)


jt.PT = Waterfall.Pseudotime(jt.l_tmp, angle=0, col = age_col_vect, plot_title = "103117_FR_removed_bad by Fetal Age", 
                             nclust = 5, seed = 5,scalePCA = F,mst=FALSE,invert_pt = FALSE, 
                             label=TRUE,threeD_plot = FALSE)

jt.PT = Waterfall.Pseudotime(jt.l_tmp, angle=0, col = group_col_vect, plot_title = "103117_FR_removed_bad by Retina", 
                             nclust = 5, seed = 5,scalePCA = F,mst=FALSE,invert_pt = FALSE, 
                             label=TRUE,threeD_plot = FALSE)
...
markers <- c("RXRG", "CRX", "ARR3","RORB", "GTF2IRD1", "NR2E3", "MYCN", "CDKN1B", "RB1", "OLFM3", "GNAT1", "NRL", "GNAT2", "THRB", "LHX2", "CCND1", "MDM2", "SYK", "OPN1SW", "VSX2", "SOX2","CRB1","CRB2","POU4F2", "PROX1", "OPN1MW","OPN1LW", "CRB3")
#not present: "POU4F2", "PROX1" "OPN1MW""OPN1LW", "CRB3"
rod_markers<-c("NRL","NR2E3", "PDE6B", "CNGA1","CNGB1", "GNAT1", "GNB1","RHO","PDE6A")
#not present:"RHO","PDE6A",
coords = Waterfall.PCA(jt.l_tmp)#change jt.l_tmp to dataset_used for specific cell group
coords = coords$pcaN[,1:2]
out_PDF="/dataVolume/storage/single_cell_pipeline/results/dominic/111517_NoPoor_Genes.pdf"
pdf(out_PDF, title="111517_NoPoor_Genes")

for (i in markers){
  TargetGene <- jt.l[getGene(i),] #for all cells, multiple genes just rerun from here down
