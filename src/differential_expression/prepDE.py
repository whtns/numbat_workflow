...
parser.add_option('-i', '--input', '--in', default='stringtie', help="the parent directory of the sample sub-directories [default: %default]")
parser.add_option('-g', default='output/gene_count_matrix.csv', help="where to output the gene count matrix [default: %default")
parser.add_option('-t', default='output/transcript_count_matrix.csv', help="where to output the transcript count matrix [default: %default]")
...
for (dirpath, dirnames, filenames) in os.walk(opts.input):
    ...
    for filename in filenames:
        if filename.endswith('.gtf'): 
            samplename = filename.replace(".gtf","")
            samples[samplename] = (os.sep.join([dirpath,filename]))
# ~ IPython.embed()
if len(samples) == 0:
  parser.print_help()
  print (" ")
...
for q,s in samples.items():
    ...
    try:
        with open(next(glob.iglob(s))) as f:
...
for q, s in samples.items():
    ...
    print (q,s)
    try:
        with open(next(glob.iglob(s))) as f: #grabs first .gtf file it finds inside the sample subdirectory
...
##                        transcriptList.append((g_id, t_id, int(ceil(coverage*transcript_len/read_len))))
                        ...
                        t_dict[t_id].setdefault(q, int(ceil(coverage*transcript_len/read_len)))
                    t_id=RE_TRANSCRIPT_ID.search(v[len(v)-1]).group(1)
...
##            transcriptList.append((g_id, t_id, int(ceil(coverage*transcript_len/read_len))))
        ...
        warnings.warn("No GTF file found in "+s)
...
##        transcriptList.sort(key=lambda bla: bla[1]) #gene_id
    for i,v in t_dict.items():
        ...
        geneDict[geneIDs[i]].setdefault(q,0)
        geneDict[geneIDs[i]][q]+=v[q]

with open(opts.t, 'w') as csvfile:
    my_writer=csv.DictWriter(csvfile, fieldnames=[""]+list(samples.keys()))
