...
steps_to_process_all = ["uniqify", "threshold_bam"]

parser = argparse.ArgumentParser(description="runs single cellk pipeline")
...
cell_name = "_".join(r1_base_filename.split("_")[0:4])
...
if("fastqc" in steps_to_process):
	...
	cmd = ["fastqc",fastq_r1_location,fastq_r2_location] #,fastq_r1_location,fastq_r2_location
	cmd += ["-t", "3"]
...
if("cutadapt" in steps_to_process):
	...
	cutadapt_arguments = ["-a", adapter_sequence, "-A", adapter_sequence, "-m", "20", "--overlap", "5"]
	cmd = [cutadapt_binary]
...
fastq_r1_trimmed_unpaired = trimmomatic_output_dir+ r1_base_filename + "_trimmed.unpaired.fastq"
fastq_r2_trimmed_unpaired = trimmomatic_output_dir+ r2_base_filename + "_trimmed.unpaired.fastq"
...
if("trimmomatic" in steps_to_process):
	...
	#cmd.append("-trimlog")
	#cmd.append(log)
	cmd.append(fastq_r1_cutadapt)
	cmd.append(fastq_r2_cutadapt)
...
hisat2_sorted_bam = hisat2_output_dir + r1_base_filename + ".bam"
if("hisat2" in steps_to_process):
	print "processing hisat2-tweaked"
	log_file = hisat2_output_dir +r1_base_filename+".hisat2.log"
	...
	cmd += ["-1",fastq_r1_trimmed,"-2",fastq_r2_trimmed, "-U", fastq_r1_trimmed_unpaired, "-U", fastq_r2_trimmed_unpaired]
...
merge_partner_bam = merge_partner_dir+ cell_name + "hisat2" + r1_base_filename + ".bam"
merged_bam= hisat2_output_dir+ cell_name + "_merged.bam"
merge_log = picard_output_dir+r1_base_filename+"_merged.txt"
...
if((("merge_files" in steps_to_process) or ("all" in steps_to_process)) & (merge_partner_dir != "none")):
	...
	out_file = picard_output_dir + r1_base_filename+"_merged.log"
	err_file = picard_output_dir + r1_base_filename+"_merged.err"
	print " ".join(cmd)
...
removed_dup_metrics = picard_output_dir+r1_base_filename+"_removed_dup_metrics.txt"
picard_tmp_dir = picard_output_dir+"tmp/"
...
if(("removeduplicates" in steps_to_process) or ("all" in steps_to_process)):
	...
	out_file = picard_output_dir + r1_base_filename+"_markduplicates.log"
	err_file = picard_output_dir + r1_base_filename+"_markduplicates.err"
	print " ".join(cmd)
	with open(err_file,"w")as outerr:
		with open(out_file,"w")as outf:
			errcode = subprocess.call(cmd,stdout=outf,stderr=outerr)
...
picard_output_dir = cell_output_dir+"picard/"
removed_duplicates_bam= picard_output_dir+ r1_base_filename + "_removed_duplicates.bam"
rnaseq_metrics = picard_output_dir+r1_base_filename+"_rna_metrics.txt"
picard_tmp_dir = picard_output_dir+"tmp/"
...
rseqc_out = picard_output_dir+r1_base_filename
if(("rseqc" in steps_to_process) or ("all" in steps_to_process)):
	...
	out_file = picard_output_dir + r1_base_filename+"_rnaseq_metrics.log"
	err_file = picard_output_dir + r1_base_filename+"_rnaseq_metrics.err"
	read_distout = picard_output_dir + r1_base_filename+"_read_dist.log"
	read_disterr = picard_output_dir + r1_base_filename+"_read_dist.err" 
	print datetime.datetime.today()
...
intronic_bam = picard_output_dir + r1_base_filename + "_unique_intronic.bam"
...
if("uniqify" in steps_to_process):
	...
	print datetime.datetime.today()
	print " ".join(cmd)
	err_file = hisat2_output_dir + r1_base_filename+"_uniqify.log"
...
intronic_bam = picard_output_dir + r1_base_filename + "_unique_intronic.bam"
threshold_bam = hisat2_output_dir + r1_base_filename + "_depth_under_3.bam"
threshold_bed  = hisat2_output_dir + r1_base_filename + "_depth_over_3.bed"
...
if("threshold_bam" in steps_to_process):
	...
	print datetime.datetime.today()
	print " ".join(cmd)
	err_file = hisat2_output_dir + r1_base_filename+"_threshold_bam.log"
