...
mergeSingleCellExperiments <- function(sce1, sce2, expids){
	...
	merge_cd <- dplyr::bind_rows(data.frame(colData(sce1)), data.frame(colData(sce2)), .id = "batch") %>% 
	...
	assays <- lapply(assay_names, sce1, sce2)
...
```{r}
feature_sces_fs <- c("~/single_cell_projects/quicklinks/FACS_20171031_dshayler_H_sapiens_proj/output/sce/gene_sce.rds", "~/single_cell_projects/quicklinks/FACS_20171031_dshayler_H_sapiens_proj/output/sce/transcript_sce.rds")

feature_sces <- lapply(feature_sces_fs, readRDS)
```
...
# save census matrix
dir.create("../output/sce")

features <- c("gene", "transcript")

census_paths <- paste0("../output/sce/", features, "_census_matrix.csv")

census_counts <- purrr::map2(feature_sces, census_paths, run_census)
...
sce_paths <- paste0("../output/sce/", features, "_sce_with_census.rds")
...
norm_sce_paths <- paste0("../output/sce/", features, "_sce_normalized.rds")
